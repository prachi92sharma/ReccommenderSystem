...
def preProcessLine(line):
    ...
    line = line.lower()
    # Replace punctuation
    line = line.replace(',','').replace('%','').replace('+','').replace('&','').replace('.','').replace('#','').replace('$','').replace('-','').replace(':','').replace('"','').replace('@en','').replace('[','').replace(']','').replace('(','').replace(')','').replace('?','').replace("'s",'').replace("\\",'').replace('/','')
    # Remove Digits 
...
for l in sys.stdin:
    ...
    if labels == 'doc_file:' or labels == 'label_file:' or labels == 'cat_file:':
        continue
    line = preProcessLine(line).split()
    tokens =  removeStopWords(line[3:])
    labels = labels.split(',')
    
    for label in labels:
        print '%s\t%s' % (label, len(tokens))